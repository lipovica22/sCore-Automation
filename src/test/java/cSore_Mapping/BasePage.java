...
public class BasePage {
    ...
    public void SelectValueAC(WebElement element, WebElement elementList, String log, String valueEnter, String valueSelect) throws Exception {
        ...
        while (retryCount < maxRetries){
            try {
                ...
                fluentWait.until(ExpectedConditions.elementToBeClickable(elementList));
    ...
    public void gridPrinting(WebElement element, String log, String print) throws Exception {
        ...
        public void gridUpload(WebElement element, String log, String documentName,String documentTypeUploaded, String Confidentialities) throws Exception {
            FluentWait<WebDriver> fluentWait = new FluentWait<>(driver)
                    .withTimeout(Duration.ofSeconds(waitTime))
                    .pollingEvery(Duration.ofMillis(500))
                    .ignoring(StaleElementReferenceException.class)
            ...
            try {
                ...
                for (WebElement row : rows) {
                    ...
                    if (data.contains(documentName)) {
                        WebElement documentRow = row.findElement(By.xpath("//td/span[contains(text(),'"+documentName+"')]"));
                        WebElement column = documentRow.findElement(By.xpath("//td[3]"));
                        SelectOption(column,"",documentTypeUploaded);


                        //WebElement printButton = row.findElement(By.xpath(".//tr[contains(text(), 'Štampaj')]"));
                       // Click(printButton, log + documentName);
